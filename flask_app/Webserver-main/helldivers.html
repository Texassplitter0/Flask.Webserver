<!DOCTYPE html>
<html lang="de">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Helldivers</title>
    <style>
        body {
            margin: 0;
            font-family: Arial, sans-serif;
            background-color: #f4f4f4;
            color: #333;
            text-align: center;
            background-image: url('images/BG-DönerHub-Siegel.webp');
            background-size: cover;
            background-position: center;
            background-repeat: no-repeat;
        }
        nav {
            display: flex;
            justify-content: center;
            background-color: #000000;
            padding: 10px 0;
            position: sticky;
            top: 0;
            z-index: 1000;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
        }
        nav a {
            color: #FFD700;
            text-decoration: none;
            padding: 10px 20px;
            font-weight: bold;
            display: inline-block;
        }
        nav a:hover {
            background-color: #444;
            border-radius: 5px;
        }
        header {
            background-color: #FFD700;
            padding: 20px;
            margin: 0;
        }
        main {
            padding: 20px;
        }
        .box {
            background-color: #333;
            color: #FFD700;
            border-radius: 10px;
            padding: 80px;
            max-width: 800px;
            margin: 20px auto;
            text-align: left;
            box-shadow: 0 2px 5px rgba(0, 0, 0, 0.1);
            background-size: cover;
            background-position: center;
            position: relative;
        }
        .box::after {
            content: "";
            position: absolute;
            top: 0;
            left: 0;
            right: 0;
            bottom: 0;
            background: rgba(0, 0, 0, 0.5);
            border-radius: 10px;
        }
        .box * {
            position: relative;
            z-index: 1;
        }
        #gameArea {
            position: relative;
            width: 100vw;
            height: 100vh;
            background-color: #000;
        }
        .bug {
            position: absolute;
            width: 50px;
            height: 50px;
            background-color: red;
            border-radius: 50%;
            cursor: pointer;
        }
        #scoreBoard {
            position: absolute;
            top: 10px;
            left: 10px;
            color: white;
            font-size: 24px;
        }
        #highscoreTable {
            position: absolute;
            top: 50px;
            left: 10px;
            color: white;
            font-size: 20px;
        }
        iframe {
            border: none;
            width: 100%;
            height: 600px;
            margin-top: 20px;
            border-radius: 10px;
        }
        .cta {
            display: inline-block;
            text-decoration: none;
            padding: 10px 20px;
            background-color: #FFD700;
            color: #333;
            border-radius: 5px;
            font-weight: bold;
        }
        .cta:hover {
            background-color: #ffb700;
        }
        footer {
            background-color: #000000;
            color: white;
            text-align: center;
            padding: 1em 0;
        }
    </style>
</head>
<body>
    <!-- Navigation Bar -->
    <nav>
        <a href="{{ url_for('welcome') }}">Home</a>
        <a href="{{ url_for('minecraft') }}">Minecraft Server</a>
        <a href="{{ url_for('gamedivers') }}">GameDivers</a>
        <a href="{{ url_for('memecoin') }}">Memecoin</a>
        <a href="{{ url_for('profile') }}">Profil</a>
    </nav>

    <!-- Header -->
    <header>
        <h1>Helldivers</h1>
        <h2 id="random-text">Platzhalter für Zufallstext</h2>
    </header>

    <!-- Main Content -->
    <main>
        <div class="box" style="background-image: url('images/BG-Salutieren-Dönerspieß-Diver.webp');">
            <h2>Live-Details</h2>
            <div id="server-status">
                <iframe src="https://steamdb.info/embed/?appid=553850" height="389" style="border:0;overflow:hidden;width:100%" loading="lazy"></iframe>
            </div>
        </div>
        <div class="box" style="background-image: url('images/community-bg-gamedivers.webp');">
            <h2>Community</h2>
            <p>Treten Sie unserer freundlichen und aktiven Community bei!</p>
            <a href="https://discord.gg/ihre-invite-id" target="_blank" class="cta">Jetzt beitreten</a>
        </div>
        <div class="box" style="background-color: #333;">
            <div id="gameArea"></div>
            <div id="scoreBoard">Score: 0</div>
            <div id="highscoreTable"></div>
        </div>
    </main>

    <!-- Fußzeile -->
    <footer>
        <p>&copy; 2025 DönerHub.de/Gamedivers.de | <a href="{{ url_for('datenschutz') }}" style="color: #ccc;">Datenschutz</a> | <a href="{{ url_for('impressum') }}" style="color: #ccc;">Impressum</a>
        <p>Disclaimer: Diese Seite ist rein für Hobby, Privat oder Satire zwecke. Ähnlichkeiten zu bereits Existierenden Websites, Personen oder Projekten, sind rein zufällig.</p>
        <p>HELLDIVERS™ 2 PRIVACY POLICYUSER AGREEMENTSoftware subject to license:HEALTH WARNINGWARNING: IF YOU HAVE A HISTORY OF EPILEPSY OR SEIZURES, CONSULT A DOCTOR BEFORE USE. CERTAIN PATTERNS MAY TRIGGER SEIZURES WITH NO PRIOR HISTORY. BEFORE USING AND FOR MORE DETAILS, SEE IMPORTANT HEALTH AND SAFETY WARNINGS ©2023 Sony Interactive Entertainment LLC. Developed by Arrowhead Game Studios AB. Helldivers is a registered trademark of Sony Interactive Entertainment LLC and related companies in the U.S. and other countries.</p>
    </footer>

    <!-- Script für zufallstext-->
    <script>
        const sentences = [
            "Incoming friendly fire! Dodge - or don't. Your call.",
            "Dispense peace with the ultimate weaponry!",
            "Return, refit, and redeploy to purge the stain of this failure with the peroxide of victory.",
            "How about a nice cup of liber-tea?",
            "Injury? What injury?",
            "Say hello to democracy!",
            "Freedom never sleeps!",
            "Also Play Minecraft",
            "Also Play Terraria"
        ];

        // Zufälligen Satz auswählen und anzeigen
        window.onload = function () {
            const randomIndex = Math.floor(Math.random() * sentences.length);
            const randomSentence = sentences[randomIndex];
            document.getElementById("random-text").textContent = randomSentence;
        };
    </script>

    <!-- Minigame Logik -->
    <script>
        const username = "{{ username }}"; // Username wird vom Server eingefügt
        let score = 0;
        const gameArea = document.getElementById('gameArea');
        const scoreBoard = document.getElementById('scoreBoard');
        const highscoreTable = document.getElementById('highscoreTable');

        function createBug() {
            const bug = document.createElement('div');
            bug.classList.add('bug');
            bug.style.top = Math.random() * (window.innerHeight - 50) + 'px';
            bug.style.left = Math.random() * (window.innerWidth - 50) + 'px';

            bug.addEventListener('click', () => {
                score++;
                scoreBoard.innerText = `Score: ${score}`;
                bug.remove();
                saveScore();
            });

            gameArea.appendChild(bug);
            setTimeout(() => bug.remove(), 5000); // Bug verschwindet nach x Sekunden
        }

        function saveScore() {
            fetch('/save_score', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({ username: username, score: score })
            })
            .then(response => response.json())
            .then(data => updateHighscore(data));
        }

        function updateHighscore(data) {
            highscoreTable.innerHTML = '<h3>Highscores:</h3>';
            data.highscores.forEach(entry => {
                const entryDiv = document.createElement('div');
                entryDiv.innerText = `${entry.username}: ${entry.score}`;
                highscoreTable.appendChild(entryDiv);
            });
        }

        // Initial Highscores laden
        fetch('/get_highscores')
            .then(response => response.json())
            .then(data => updateHighscore(data));

        // Start des Spiels
        setInterval(createBug, 1000);
    </script>
</body>
</html>
